...
- **Platform and distribution-channel dimensions** (settled 2026-08-07, closing issue #710): two dimensions rather than one. **Platform** is the runtime surface, resolved from the BUILD MODE first and the §2 port's `isNativeWrapper` flag only after it: `desktop` for mode `desktop`, `ios` for mode `native` or for any other mode whose port claims the flag, `twa` when the start URL's query parameter is present (the E2a story owns overriding the Bubblewrap `twa-manifest.json` start URL to carry it; the web manifest's own `start_url` is `./` and carries nothing), `web` otherwise. Mode first because the flag alone cannot tell the two wrapper shells apart, and because a desktop shell may inject no port at all. **Distribution channel**, emitted as the property `distribution_channel`, is the commercial channel: `web`, `twa`, and `ios` carry their platform's name, and a `desktop` build reports the storefront its shell stamped on the optional port member `channel`, accepted only as exactly `steam` or `itch` and reported as `unknown` otherwise. The emitted property carries the `distribution_` prefix because a bare `channel` collides with PostHog's built-in `$channel_type` (direct / organic / paid / referral) in the property picker; the PORT member keeps the short name, since that is the cross-repo contract the private shell implements and the collision is only on the analytics side. Splitting the two dimensions keeps one desktop runtime serving several stores readable as a filter rather than a re-labelling. Nothing is reported from a wrapped build today because the telemetry gate is closed for them; opening it is a later stage of the desktop analytics epic (#781). Milestone events hook only existing UI/controller-layer notifications; nothing under `src/engine/` is instrumented.
