...
- **F1. Native-wrapper detection.** The web app can tell at runtime whether it is running inside a bundled native wrapper, through a single platform flag the wrapper shell sets and the browser build leaves unset. The flag is available to both wrapped build modes (amended 2026-07-29: `--mode native` for the iOS Capacitor shell, `--mode desktop` for the desktop Electron shell). E3b specifies the iOS shell setting it; whether the desktop shell uses this seam is part of its own design. The Android TWA deliberately runs the plain web build (that is how it stays current with the site), so the flag stays unset there.
- **F2. Service-worker gating.** In bundled wrapper builds (amended 2026-07-29: `--mode native` for iOS and `--mode desktop` for the Electron shell), the service-worker registration and the hourly update poll (`src/pwa.ts`) do not run; those builds update through the store or the wrapper's own channel. The Android TWA keeps the full web update flow, service worker included, because it renders the live site.
...
- **F8. Anonymous usage analytics.** The game reports anonymous, aggregate usage events (app opens, session length, a small set of gameplay milestones) through a cookieless analytics integration in the Plausible/Umami class whose data is collected solely for this project (never used or shared by a vendor for its own purposes): no cookies, no stored personal identifiers, no cross-site tracking, a posture designed to require no consent banner and no Apple App Tracking Transparency prompt (the implementing story validates that this holds against current store policy and applicable law before any store declaration relies on it). The transport necessarily carries IP address and user agent; the requirement includes their handling: the chosen tool must discard or anonymize them after transient processing, and a self-hosted server's access logs must be disabled or IP-anonymized, so nothing personal is retained anywhere. Whether the endpoint is first-party in the browser sense depends on the story-time hosting choice; self-hosting on the game's origin also reduces ad-blocker loss and is the preferred option. One integration in the shared web codebase covers every surface (the plain build for web and TWA, which renders the live site; the wrapped builds for iOS and the desktop shell); the endpoint is an absolute URL (relative paths break under the Capacitor scheme) and the client script is bundled, not CDN-loaded. Dev, test, and CI runs send nothing by default; local validation uses an explicit opt-in (E1e's acceptance criteria). Every event carries two dimensions (settled 2026-08-07, closing issue #710): a **platform** dimension for the runtime surface, keyed on the build mode first and the F1 flag only after it (`desktop` for the Electron build, `ios` for the iOS build or any other build whose port sets the flag, `twa` via a query parameter on the Bubblewrap start URL since the web manifest's own start URL carries none, `web` otherwise); and a **distribution channel** dimension for the commercial channel, emitted as the property `distribution_channel` (`web`, `twa`, and `ios` follow the platform; a desktop build reports `steam` or `itch` from the storefront its shell stamped, and `unknown` for anything else). Two dimensions rather than one because a single desktop runtime serves several storefronts, and the flag alone cannot tell the two wrapper shells apart. Both the emitted property and the port member the shell stamps (`distributionChannel`) carry the prefix, because a bare `channel` collides with the analytics tool's own built-in channel-type property, which means marketing attribution. Nothing is reported from a wrapped build today; opening the gate is a later stage of the desktop analytics epic (#781). Tool choice and self-hosting decision happen at story time; if self-hosted, the hosting config is ops and lives in the private repo. Store privacy declarations (Play Data Safety, Apple privacy nutrition labels) must describe this posture truthfully; the E2d/E3d store stories own that alignment.
