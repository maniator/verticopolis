...
| 26 | Restaurant and fast food each got more one-shot variety; every scene's room-tone bed moved from a mid bandpass to a warm steep lowpass. | Owner playtest after v1.62.0: food places "still feel a bit repetitive" (one or two one-shot types on a steady cadence) and "some places still feel a little staticish" (the per-scene beds were mid-band bandpasses). Approved against A/B preview WAVs: restaurant grew to three one-shot characters (one a two-note tinkle), fast food to a clustered tray plus a pitched register and counter thud, and every scene's bed became a steep rolloff -48 lowpass at a warm cutoff (food 720 Hz) seated about 2 dB lower. The same-session `/gds-code-review` caught two real defects in the first cut: the single-bed constructor retune was overwritten per scene by `applyScene` (so the fix had to move to `SCENES[].amb`), and a long fast-food noise texture was chopped on the shared mono noise voice (so the extra fast-food textures were made pitched, leaving the tray as the one noise element). Also capped the paired-ping fundamentals so the companion note stays under 1900 Hz. Ships v1.63.0. |

| 27 | Human-voiced jingles, round one (amendment 2026-08-06): bloop replaces `build` at every call site; a new `notify` cue (single ping bell) takes the onboarding chime; `promote` keeps its five-note melody re-voiced on the ping's bell timbre for star promotions and the win; `sell`/`error`/`click` wait for the theme pass; SFX bus stays dry (a reverb send would escape the Effects slider); player notes state the sounds are made from the owner's own recordings, human-made, not AI-generated. | Owner recorded two sounds with their own voice ("bloop" sine drop 520→180 Hz, "ping" G4 bell with a quiet D6 partial), approved the synthesized versions by ear, and asked for party-vetted integration. Party round (Samus, Sally, walk-ons): falling contour ruled a placement gesture; a fanfare per tutorial step ruled inflation; the promotion moment kept its size via the carillon compromise. |
