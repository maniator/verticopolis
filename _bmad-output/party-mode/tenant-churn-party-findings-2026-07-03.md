...
| 7 | Reconsider counting `vacating` toward the **star** population | 🔲 Open — see below |
| 8 | Let office **noise** actually evict (re-add the `noise` cause) | 🔲 Open — deferred mechanic |
| 9 | Confirm the amber ribbon doesn't churn demo **screenshots** | 🔲 Open — verify |
| 10 | **Playtest** the churn feel (teeth, not tedium) | 🔲 Open — needs a human |

## Open follow-up backlog (the "other findings")

### F-7 — Should a tenant *on notice* still prop a star gate? (parity judgment)
`isPresent` counts `vacating`, so a unit actively leaving still credits the star
rating. Combined with a tower that keeps units chronically marginal, this can
hold a tower at a star threshold on tenants who are perpetually "leaving." It is
**documented** (arch invariant #4) but not **decided**. Options: (a) keep as-is —
they haven't left yet, so they count (current); (b) exclude `vacating` from
`ratingPopulation` only, so churn risk visibly threatens your star. *Needs a
parity/design call before it's worth code.* — flagged by Samus & the game-design
review.
...
### F-9 — Screenshot stability of the on-notice ribbon
`gdd-legibility` principle #5 says nothing new should recompute on the tick loop
for the demo screenshots. The amber ribbon is *static* per unit, but it
appears/disappears as satisfaction crosses zero. Verify the screenshot suite
(`npm run screenshots:docker`) is stable, or gate the ribbon out of the demo
capture. — Sally.
