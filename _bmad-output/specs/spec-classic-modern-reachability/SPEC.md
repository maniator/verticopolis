...
- **Targeted routing only.** Two explicit Vercel rewrites (`/help`, `/gallery`); never sitewide `cleanUrls`. `/help` adds one Vite input, the precached `help.html`, and one `navigateFallbackDenylist` entry; no client-side router, no Workbox `runtimeCaching`, no self-registering SW on the pages (the root-scope game SW covers them), no SSR/prerender. (The clean-URL offline precache alias this spec originally prescribed is deliberately omitted, see CAP-5: a generateSW alias would fetch `/help` at SW install and 404 off-Vercel, failing the whole precache.)
