...
- **Targeted routing only.** Two explicit Vercel rewrites (`/help`, `/gallery`); never sitewide `cleanUrls`. `/help` adds one Vite input and one precache entry and one `navigateFallbackDenylist` entry; no client-side router, no Workbox `runtimeCaching`, no self-registering SW on the pages (the root-scope game SW covers them), no SSR/prerender.
