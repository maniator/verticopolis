...
- **CAP-5: a shareable standalone `/help` page**
  - **intent:** Anyone can open and share a canonical Classic vs Modern reference at a clean URL, and always get back to the game.
  - **success:** `src/help.html` is a new Vite input rendering `compareTemplate()` inside the retro page shell (CAP-6), served at the clean `/help` via one targeted Vercel rewrite, with SEO/OG head parity and Vercel telemetry parity with the game. Workbox precaches the built `help.html`, and the clean `/help` is registered as a precache alias for it so the clean URL resolves from cache offline (not just online), with `/help` on the `navigateFallbackDenylist` so the `index.html` app shell does not hijack it. "Back to game" is a real `<a href="/">` in the title bar and footer. The in-modal "Open full page" link is a real `<a href="/help" target="_blank" rel="noopener">` that a click handler downgrades to opening the in-app modal when the app is an installed standalone PWA (`matchMedia('(display-mode: standalone)').matches || navigator.standalone`). All per-file detail (Vite input, precache, `navigateFallback`, the rewrite, canonical/sitemap, the standalone fallback, back-to-game, and telemetry) is specified in **page-architecture.md**.
...
- **Targeted routing only.** Two explicit Vercel rewrites (`/help`, `/gallery`); never sitewide `cleanUrls`. `/help` adds one Vite input, the precached `help.html`, a precache alias mapping the clean `/help` to it (generateSW `additionalManifestEntries` / `manifestTransform`, no injectManifest), and one `navigateFallbackDenylist` entry; no client-side router, no Workbox `runtimeCaching`, no self-registering SW on the pages (the root-scope game SW covers them), no SSR/prerender.
...
- Sitewide `cleanUrls`, Workbox `runtimeCaching`, a page-registered service worker, SSR, or prerendering.
