...
- **One new cache only: a lazy revision-keyed noise memo** living beside `noiseAfflicted` in `satisfaction.ts` (roundtable refinement 2026-07-15, superseding the earlier eager `noiseIndex.ts` module). On a query miss it computes through the EXISTING `nearestKindWithin`, so the scan semantics (multi-story cinema source, lobby-shield order, band edges) keep one textual home and cannot drift; the first boundary after a player edit pays one fresh sweep (no worse than today) and watched hours are cache hits. The served-floor set and transport columns are already revision-memoized in shipped code; their measured cost is call-path overhead, recovered by hoisting `tower.servedFloorSet()` out of the per-unit loops in `updateSatisfaction`, `attemptMoveIns`, and `ElevatorDispatch.accumulateWaiting` (the last also trims the flat minute tick), and by passing `farWalk`/`noisy` through to `vacateCause` instead of recomputing.
- **Cache storage is a Simulation transient** (the `elevatorUtil` precedent; noise is a sim rule, and Tower's existing revision caches are routing derivations), revision key compared with strict equality and initialized to -1. A module-level singleton keyed by bare revision is forbidden: revision is a per-instance counter and two Tower instances collide. `deserialize` builds a fresh Simulation, so a stale memo cannot survive undo or load.
- **Never cache state-reading predicates.** The `functionalParkingSet` precedent (`src/engine/tower/routing.ts:118-125`) is the fence: fire, gut, construction-finish, and occupancy mutations do NOT bump revision. `noiseAfflicted` is cacheable only because it is state-blind, including that a gutted or empty room still radiates by kind; that behavior must not be cleaned up in passing.
...
- **Mandatory differential test.** On randomized towers, every sensitive unit asserts memoized noise flag === a fresh cache-bypassed `nearestKindWithin`, covering: the multi-story cinema upper-story source (`register` maps both stories into `rooms`), lobby-shield break BEFORE the source check, open-air gap break, shared-wall distance 0, and exact band edges 11/21. The lazy memo computes through the same function, so this is insurance against future divergence rather than a guard on a reimplementation. Plus state-blindness pins (fire/gut/finish/occupancy leave cached === fresh) and a mutator sweep asserting all eleven `revision++` sites bump (place, removeUnit, placeTransport, removeTransport, resizeTransport, setCars, setStop, clearStops, setExpressStops, coerceExpressStops, reindex).
