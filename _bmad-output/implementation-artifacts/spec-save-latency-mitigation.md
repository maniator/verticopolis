...
Routine autosave should avoid the measured synchronous DEFLATE hitch on large towers while preserving the existing save format and the synchronous durability path used before reloads.
</frozen-after-approval>
...
- `src/tests/storage.test.ts`: cover async save round-trip and fallback behavior.
