...
| 2026-07-16 | attendance-finish-tripwire | — | gameplay-feel | test-guard | P1 | med | — | done | **IMPLEMENTED (issue #302); issue closes on merge.** The despawn choke point is guarded twice in `src/tests/integration/attendanceTripwire.integration.test.ts`: a source-level tripwire (the only `state = "done"` write across the crowd modules lives inside `finish()`, so a new despawn shortcut fails the scan before it can leak a tally) and a runtime reconciliation invariant (every unit's `customersIn` / `hotelCustomersIn` / `outForMeal` equals the count of live people carrying the matching stamp, checked across a mixed day, a full overnight drain, and a bulldozed origin). The guard core shipped 2026-07-15 with the attendance-guards PR; this pass completes the issue's scenario walk with the mid-visit teardown paths: venue bulldozed under counted visitors (the rebuilt unit takes no ghost decrement), fire and gutting mid-show (tallies drain through `finish()`, the occupants mirror holds at 0 on the non-operational shell), and a save/load round-trip taken mid-visit (fresh crowd, every transient tally restores to zero, forged saves stripped). `/gds-code-review`. |
