...
| 2026-07-17 | schedule-origin-accumulator | — | elevator-scheduling | feature-request | P3 | — | — | done | **IMPLEMENTED (issue #465); issue closes on merge.** The measured accumulator is per-shaft per-hour only, so the origin-floor Simulate variants ("down-rush originates on floors 30-38"), the §15 peak-origin staging markers, and a demand-aimed Auto-tune seed are all unbuildable; the Simulate copy was re-pinned to the staging-scored sentence meanwhile. Build a transient per-shaft ring of hourly boardings bucketed by origin floor band, sampled where `sampleElevatorUtil` already walks shafts. MUST stay transient (a persisted accumulator is a save-format conversation this row does not authorize). No RNG; golden master untouched. `/gds-code-review`. Shipped in v1.60.0: a boarding tally at the dispatch board site drained hourly into day-split per-hour origin maps, hotspot markers in the floors grid, the Simulate boarding clause, and the origin-aimed Auto-tune staging seed. |
