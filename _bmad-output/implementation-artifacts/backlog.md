...
| 2026-07-29 | report-leash-survives-tower-swap | #684 | title-screen-load-tower | review-deferral | P2 | med | — | idea | **Deferred from `/bmad-code-review` of the modal-precedence work (PR #683, v1.106.0).** SPEC-modal-precedence-reports CAP-5 names a tower swap as one of three things that must BREAK a waiting report's leash; the shipped seam breaks on displacement and on a second report, but not on a swap. The New Tower dialog is protected, so a report waits behind it, and its `found` handler calls `ui.closeModal()` then swaps the simulation: the close RESOLVES the leash, so the stale report opens over a brand-new tower and "Open tower" would adopt a tower parsed against a state that no longer exists. Fix: capture a swap generation when the wait is set and break rather than resolve when it no longer matches, adding a `"tower-swapped"` `BrokenReason` so the message names the real cause. The broken path and its copy already exist. `/bmad-code-review`. |
| 2026-07-29 | open-report-clobbered-silently | #685 | title-screen-load-tower | review-deferral | P2 | med | — | idea | **Deferred from `/bmad-code-review` of the modal-precedence work (PR #683, v1.106.0).** Found independently by the Blind Hunter and the Acceptance Auditor. A WAITING report now fails loudly, but a report already on screen has no protection: any dialog opening over it wipes the parsed tower silently (`openModalTemplate` calls `replaceChildren()`), reachable from the frame loop surfacing a second emergency or the congratulations dialog on a later frame. The report is marked protected, but only the two report entry points consult protection, so the label does not defend it. This makes the 1.106.0 CHANGELOG line ("it says so in the window you are looking at") true of the waiting case only. Fix: re-arm a leash when an open report is displaced so the existing broken path fires. Making every opener consult `ownsPendingWork()` would be a general precedence rule, which the spec lists as a Non-goal. `/bmad-code-review`. |
| 2026-07-29 | import-failure-toasts-under-dialog | #686 | title-screen-load-tower | review-deferral | P3 | low | — | idea | **Deferred from `/bmad-code-review` of the modal-precedence work (PR #683, v1.106.0).** CAP-1 requires that NO refusal or deferral path on this flow reports itself only through a toast raised while a modal is open. PR #683 fixed the precedence branch; the sibling failures on the same async flow still toast: `src/ui/uiImport.ts` `reader.onerror`, and the `importLegacy` parse failure, `importGame` failure and `exportLegacy` failure in `src/game/saveLoad.ts`. Each is a refusal on exactly the #658 race, landing under a dialog that paints over it. Fix: route them through `routeNotice`, which already falls back to a toast when nothing is on screen; that also makes the fallback branch reachable and testable. `/bmad-code-review`. |
