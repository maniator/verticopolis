...
| 2026-07-28 | landing-tile-router-parity | #674 | pixel-art | review-deferral | P3 | low | — | idea | **Deferred from the `/gds-code-review` of the stair-reachability PR (v1.104.1), Blind Hunter finding.** Not a live defect; the two agree today by construction. `landingTile` (`src/render/excalibur/towerWalkerBuild.ts`) is a second, independent copy of the router's `landingSeg` rule (`src/engine/tower/segments.ts`): both scan the footprint left to right for the first structural column and both fall back to `t.x`, so `positionReachable(floor, landingTile(...))` lands on the segment `landingSeg` would pick. Nothing pins that correspondence: every `landingTile` case is checked against a hand-written fixture, never against `landingSeg` on a real `Tower`. How it breaks: `segment-shaft-multi-run` (#662) records that `landingSeg` links only the FIRST of two runs an overlapping footprint straddles. If #662 is fixed by returning all overlapped runs and letting the router link the reachable one, `landingTile` keeps returning the first column, which may sit on the stranded run, and every climber on a routable stair disappears again (the exact bug #665 clause 2 fixed), with no test failing. Fix: assert `segAt(tower, floor, landingTile(tower, t, floor)) === landingSeg(tower, t, floor)` on a real gap-split `Tower` (`buildWalkers` runs under happy-dom, so this is a unit test). Or remove the duplication by having the engine expose the landing TILE beside the segment, which widens the render-facing surface and so wants the "Ask First" seam. `/gds-code-review`. |
