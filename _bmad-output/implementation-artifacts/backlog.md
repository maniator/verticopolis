...
| 2026-07-21 | save-integrity-bundle | #537 | save-hardening | bug | P2 | med | — | open | **From full-repository-audit 2026-07-21 (`_bmad-output/planning-artifacts/reviews/full-repository-audit-2026-07-21/`), findings AUD-010/011/012 (N-4 bundle; one PR).** Manual quick-save and slot-save swallow storage-quota failures behind a success toast (`src/game/saveLoad.ts:112-116`, `src/game/appModals.ts:71-77`; silent data-loss risk; the quota spec deliberately covers only the reload path); `Simulation.deserialize` polices transport overlap but not room-unit overlap (forged save corrupts the per-tile room index, the exact class `src/storage/tdtParse.ts:133-136` drops on import); and `data.units` has no count cap while transports do (crafted-save load-time hang). Fixes and acceptance in the issue. Excluded on purpose: the wider forged-save permissiveness posture (owner decision D-2, audit §19). `/gds-code-review`. |
...
| 2026-07-21 | ci-hardening | #540 | — | task | P2 | med | — | open | **From full-repository-audit 2026-07-21 (`_bmad-output/planning-artifacts/reviews/full-repository-audit-2026-07-21/`), findings AUD-027/028/029/009 (N-6 with N-3 merged in; one workflow-surface PR).** SHA-pin every action (currently all on mutable tags, incl. write-scoped jobs); `persist-credentials: false` + explicit push on both baseline-minting workflows (they run branch code with a persisted write token; screenshot-capture already documents the pattern); permissions block on test.yml; perf-gate epsilon (failed a docs-only diff by 4e-13 ms at the exact tolerance boundary, CI run 29863943618); wire `RUN_HOUR_BENCH` into a scheduled non-blocking workflow (party commitment, audit §14a). Tooling ride-alongs per audit §17/§18: AUD-005 (local `npm run e2e` writes host-rendered pixels into the tracked `docs/screenshots/milestones/` gallery via `e2e/milestones.spec.ts:43`; local runs must leave `git status` clean while CI's artifact upload keeps its captures) and AUD-033 (a non-empty `ONLY=` screenshot filter matching zero scenes must exit nonzero, not report success); plus PROD-003 from the live supplement (the PWA precache manifest lists 5 icon entries twice; Workbox dedupes at install, so the build log's 26 entries are really 21; likely an includeAssets/glob overlap in vite-plugin-pwa). `/bmad-code-review`. |
...
| 2026-07-21 | a11y-ux-sweep | #541 | UI legibility | bug | P2 | med | — | open | **Curated home for the already-open a11y/UX small-fix sweep (#541), reified by #545.** The issue was filed naming "Backlog row `a11y-ux-sweep`" but no row was ever added, so the deferral-inbox items folding into it had a live issue but no curated destination. #541's canonical eight: toast rail `aria-live` interrupt (AUD-020), undo/redo bypassing the splash/modal guards (AUD-021), repeated `announce()` never re-firing (AUD-022), non-finite `accMinutes` freezing the sim (AUD-024), `.btn.xs` sub-24px touch targets (AUD-025), `#tower-name` missing an accessible name (PROD-001), `#stat-money` failing AA contrast (PROD-002), and the deprecated iOS meta tag (PROD-004). #545 folds in adjacent a11y items surfaced in review: exposing the shipped `colorblindCue` pref in Settings, a splash reduced-motion affordance reachable before dismiss, the traffic-chip `aria-live` jitter on near-tie crossings, and announcing the batch bulk-reset arming to screen readers. `/bmad-code-review` (run `/gds-code-review` too if the undo-guard reorder proves engine-adjacent). |
...
- **9 newly tracked** curated rows. Eight bundle a recurring orphaned theme with
  no prior home, each with a newly created mirrored issue:
  ...
  `audio-first-cue-latency` (#554), `lit-string-builder-retirement` (#557). The
  ninth, `a11y-ux-sweep` (#541), reifies a pre-existing open issue that named
  its backlog row when filed but never had one added, so the a11y items folding
  into it now resolve to a real curated destination.
...
the `a11y-ux-sweep` (#541) row this pass added. No pre-existing curated row was
rewritten; the table changed only by appending new rows.
