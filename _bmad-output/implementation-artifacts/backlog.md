...
| ---- | ----- | -- | ---- | ---- | -------- | -------- | ----- | ------ | ----- |
| 2026-07-30 | deserialize-unit-width-clamp | #731 | sky-lobby-over-floor | review-deferral | P3 | low | — | open | **Deferred from the /gds-code-review of the sky-lobby-over-floor change.** `Tower.place()`'s in-place floor→lobby upgrade removes the whole underlying floor `Unit` under each of the lobby's width-1 tiles; a floor unit with width > 1 loses its tiles outside the lobby span, orphaning structure (`isSupported` not re-checked). Unreachable in normal play (`FACILITIES.floor.width === 1`); requires a forged/legacy save, because `deserialize` clamps unit width only to lot bounds (`serialization.ts:193`), NOT to catalog width like transports (`:391`). Pre-existing at floor 1's upgrade; the sky-lobby change widened reachability to 15/30/45/60/75/90. Fix: clamp unit width to `FACILITIES[kind].width` in `deserialize`. Forged-save-only, non-blocking. `/gds-code-review`. |
