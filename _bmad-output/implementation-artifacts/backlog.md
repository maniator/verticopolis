...
| ---- | ----- | ---- | ---- | -------- | -------- | ----- | ------ | ----- |
| 2026-07-07 | tdt-importer | OpenSkyscraper docs | feature-request | P2 | — | — | shipped-v1 | **v1 shipped (PR 3, v1.10.0): `src/storage/tdtFormat.ts` (bounds-checked walker) + `src/storage/tdtImport.ts` (mapper, transport synthesis, fidelity report), wired through `SaveLoad.importLegacy` and `UI.showImportReport`; synthetic-fixture tests in `src/tests/tdtImport.test.ts`.** Sub-items DEFERRED to a follow-up PR: real elevator-block decode, stairs decode, named-tenant import, `rentClass` mapping, retail-subtype adoption on import, finance-block import, and the TDT-floor-110 ambiguity (uniform `ours = tdt − 9` clashes with the doc's "110 = 100F"; v1 drops-those-rows-with-report — clamping them onto floor 100 would overlap real floor-100 rooms; revisit against a real save). v1 layout assumptions to validate against a real save: the 18-byte tenant record's field packing (order from the doc; exact offsets unconfirmed) and the 14-byte person-record guess (tail is walked tolerantly, so a misfit only degrades warnings/retail rows, never the import). |
...
- **Structure-aware shaft placement for imported towers** — `synthesizeTransports`
  ...
  covers the chosen column. (Edge Case Hunter, .TDT importer review.)
