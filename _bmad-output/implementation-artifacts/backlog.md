...
- **Attendance venues (cinema/partyHall) distort the retail demand pool (Edge Case Hunter, medium; GDD-flagged revisit).** In `src/engine/sim/demand.ts` a cinema (`dailyTrafficIncome` 8000) and partyHall (3000) enter `totalCap` as capacity sinks, so a single cinema in a modest tower collapses `share = pool / totalCap` and can zero genuine retail (shop/food) income, even though a cinema's real patronage is driven by the separate live-attendance system, not the office/condo/hotel demand pool. This is the exact case `gdd-commercial-demand-pools-2026-07-15.md` §4.5 kept in the pool for v1 and marked "revisit only if a single cinema distorts a small tower at calibration"; the adversarial review confirms it distorts, so the revisit is now due. The Acceptance Auditor notes the current behavior is per-spec (intended capacity-sink design), so this is a balance refinement, not a correctness bug. Recommended fix: give attendance venues their own demand fraction from their live attendance fill (`customersIn / attendance` cap) and drop them from the retail `totalCap`, so they neither dilute retail nor draw the office-worker budget; verify against the blockbuster economics in `economyDepth`. Its own PR under `/gds-code-review`.
