...
| ---- | ----- | -- | ---- | ---- | -------- | -------- | ----- | ------ | ----- |
| 2026-07-29 | desktop-command-seam | #716 | desktop-shell | review-deferral | P3 | low | — | open | **Ready.** Deferred from the /gds-code-review of the desktop command seam (PR #715). Five sources answer "is the game interactable": app.shownChoice, app.shownUpdate, #modal.open, #splash, #crash-screen; GameApp carries 27 mutable public fields. That scatter caused a real defect this review found: the availability push sat below runFrame's early return on shownChoice/shownUpdate, so the shell was told everything was available in exactly the state where #modal.open guaranteed refusal. Fixed at the instance in #715; this row is the class. Also three hand-rolled computed-selector-with-change-detection implementations (UI.paletteScanKey, lastUiUpdate, lastAvailabilityKey). Party ruled out a state library (engine is framework-free and Classic is byte-locked; adoptSim depends on friend-modules re-reading app.sim per call; all five sources are UI chrome, not sim). Fix shape: one interaction-state module with a change notification, owning all five sources and collapsing the three dirty-gates. Only worth doing if it owns all five, since owning two adds a sixth source. Needs a design doc first. No engine, save, or gameplay impact. /bmad-code-review. |
...
| 2026-07-29 | desktop-help-page-link | — | desktop-shell | bug | P2 | low | — | resolved | **RESOLVED INVALID 2026-07-29; issue #720 closed as not-a-bug.** Reported as "the Help dialog's Open the full help page link is silently dead in the Electron shell". It is not. `uiDialogs.showHelp` installs a click handler that calls `preventDefault()` and opens the in-app Classic vs Modern comparison whenever the session is an installed standalone PWA or a native wrapper, keeping the player in their running tower instead of dropping them into a system browser. Verified against the packaged exe: the modal heading changes to "Classic vs Modern". The original probe watched window count, page URL, and console errors, every one of which is correctly unchanged when that handler fires, and never checked the modal content where the effect actually lands. The companion claim that the shell's refusal record has a blind spot was wrong for the same reason: nothing was blocked, because `preventDefault()` meant Chromium never attempted the open, so `setWindowOpenHandler` was correctly never consulted and the smoke's empty `windows` bucket is as strong as it reads. Kept as a row rather than deleted so the finding is not re-filed. The one surviving fact moved to #718: `/help` and `/gallery` are extensionless routes that resolve to no file under the `app://` handler, which constrains any future second-window work. |
| 2026-07-29 | help-gallery-secondary-window | #718 | desktop-shell | feature-request | P3 | low | — | open | **Needs a spec.** Owner proposal 2026-07-29: open Help (and add Gallery) as a real secondary window in the desktop shell, so reading either does not cover the tower, plus make "Back to game" close that window rather than navigate. Findings that shape it: (a) `/help` and `/gallery` already exist as standalone prerendered pages (`dist/help.html`, `dist/gallery.html`), so no new content is needed, and Gallery has NO in-game dialog at all, only the page, so a Gallery menu item cannot route through the command seam the way the other eight commands do. (b) Navigating the MAIN window to either page would destroy the running sim, and the shell's `will-navigate` guard would permit it because both are same-origin, so a second window is the only safe shape. (c) The Help link already downgrades in a wrapper (`showHelp` intercepts it and opens the in-app comparison), so this feature has to decide whether to keep that or route to the new window, since the label promises the full page and the fallback does not deliver it. (c2) `pageShell` renders "Back to game" as `<a href="/">`, which in a popup would boot a SECOND game instance inside the popup rather than returning to the first, so the page shell needs to learn it is in a secondary window and close instead. (d) PREREQUISITE: the shell's `will-navigate` and `setWindowOpenHandler` guards are attached to `mainWindow.webContents` only, so a second window would inherit none of them. A global `app.on("web-contents-created")` guard (plus `will-frame-navigate`, `will-redirect`, and explicit `webviewTag: false`) has to land first, and is worth doing regardless of this feature. (e) Keep the in-game Help dialog as the default on every platform: a second OS window is worse on a handheld and with a controller, and removing the dialog would fork the interface shared with web, Android, and iOS. So this is additive, desktop-only, and opt-in. Belongs to the private distribution repo's shell work plus a small public change for the page-shell close behavior. `/gds-code-review` for the public half. |
