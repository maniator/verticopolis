...
| 2026-07-20 | zero-population-tdt-renders-black | #510 | SimTower parity | bug | P3 | low | — | idea | **Found 2026-07-20 (harness): a .TDT exported with ZERO population renders a BLACK frame in the real game (9/9 attempts); any seeded population renders fine.** The 0-pop export verifies clean and differs from the occupied version only in ~1462 occupancy/people bytes, so our exporter likely writes a people/occupancy block the game cannot paint for the 0-population case (the game itself renders a fresh 0-pop tower, so it is our export). Low priority (only bites synthetic all-vacant test towers; real saves carry population). RE the people-block header/count for 0-population against a game-written save. Harness-testable. |
