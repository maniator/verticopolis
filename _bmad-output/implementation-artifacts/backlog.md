...
| 2026-07-07 | palette-unlock-visibility | — | review-deferral | P3 | low (cosmetic) | — | open | **Gated — unreachable in normal play.** After the palette hides locked facilities (`.pal-item.locked` → `display:none`), a build tool selected at a higher star can be re-locked only by loading a *lower*-star save while it is still active (`adoptSim` swaps in a lower `sim.star`; in-session star is monotonic — `Simulation.ts:1666`). The button then keeps its `.active` class while `display:none`, and `toolInfo` still describes the now-locked facility — a phantom selection. **Not a placement hole:** every build path re-checks `isUnlocked` and rejects with the "unlocks at N★" reason (`main.ts`, `buildActions.ts:76`, `Simulation.ts:440/528`). Fix if ever actioned: on lock, if the locked kind is the active tool, fall back to Inspect. (gds-code-review Blind + Edge Case Hunters, palette-unlock-visibility.) |
