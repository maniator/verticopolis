...
| 2026-07-23 | posthog-error-tracking | #611 | analytics-posthog-migration | design-decision | P3 | — | — | idea | **Deferred from the app-chrome telemetry design party (CAP-4).** The app-action sweep instrumented the app chrome but left OUT the deep per-facility editor economy actions (sell, rent up/down, price rung, add/remove car, film policy, variety, batch pricing, extend, elevator schedule) that funnel through the one `editorActions.handleEditAction` choke. Party ruling (Samus + Cloud): these are core gameplay-economy signal deserving their own story and `/gds-code-review`, not bolted onto app-chrome `/bmad-code-review`. When picked up: model the events (parametrized `editor_action`, or an `edit_*` namespace on `app_action`), wire the single choke plus the batch-pricing and elevator-schedule sub-dialogs, keep the cookieless posture (aggregate/per-session, never a player-authored value). `/gds-code-review`. |
