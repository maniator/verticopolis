...
| 2026-07-28 | import-report-lost-under-reopened-dialog | #658 | title-screen-load-tower | review-deferral | P3 | low | — | idea | **Deferred from the `/bmad-code-review` on the Load Tower PR (v1.101.0), Edge Case Hunter finding.** Pre-existing race, newly reachable from the title screen. `showImportReport`/`showExportReport` bail with a toast ("Close the open dialog first, then import again.") when `ui.isModalOpen()` is true, but a `<dialog>`'s top layer paints over the toast rail, so that toast is invisible whenever the thing blocking it is a modal. Sequence: Load Tower, Choose file, pick a `.TDT` (the picker closes, the OS picker opens, `FileReader` runs async and can take seconds on an Android cloud provider), the player re-opens Load Tower, the read completes, the report bails and the import is discarded with no feedback at all. Reachable from the in-game saves manager the same way. Fix options: queue the pending report and show it when the blocking modal closes, or let a non-blocking dialog yield to the report rather than refusing it (the emergency/update modals must still win). `/bmad-code-review`. |
