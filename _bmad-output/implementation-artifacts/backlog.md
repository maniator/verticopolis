...
### Deferred from: `bmad-code-review` of the help-copy Modern update (2026-07-20)

- **The elevator-schedule Modern authoring affordance (#305) is not called out in the "Classic vs Modern" help copy (Edge Case Hunter, low).** `GameRules.elevatorScheduleUX()` gives Modern presets, auto-tune, and advice while Classic is raw-grid-only, a genuinely player-visible mode difference, but it carries no line in the help section (classified `null` in the `RULE_TO_HELP` drift map as a UI-only affordance). Defensible: the section is gameplay-rule focused and this is a dialog affordance. Reconsider if a player reports confusion about why the schedule dialog differs by mode. If added, give it a distinct phrase and flip its `RULE_TO_HELP` entry off `null`. No other review finding deferred: shared-phrase weakness and the overstated drift-guard comment were patched; the "before they go" wording was kept (it conveys the same-day checkout).
