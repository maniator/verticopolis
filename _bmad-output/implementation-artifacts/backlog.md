...
| 2026-07-28 | walker-reachability-refinements | #665 | pixel-art | review-deferral | P3 | low | — | idea | **Deferred from the `/gds-code-review` of the ghost-crowd PR (v1.104.0), Blind Hunter finding.** Two fail-safe refinements to the #639 walker reachability gate, neither a live defect. (1) A stair/escalator climber carries its transport's BOTTOM floor (`spawnWalker(..., t.bottom, t.x, false)`), so the gate asks only about the bottom: a flight whose bottom is reachable but whose top is not still shows climbers. Reaching that state needs Classic's walk budget to cut the flight in one direction (the passenger router itself walks stairs, so a reachable bottom normally implies a reachable top), and it errs toward showing rather than hiding. Fix: gate on either endpoint, which needs the transport's top floor carried on the `Walker` too. (2) The gate is per FLOOR, but #647's segment routing (PR #652) makes a gap-split Modern floor able to strand one contiguous run while a sibling run of the same floor still routes to the lobby, which a floor-level probe cannot see. `Walker.tileX` is already carried for this and is deliberately unread today; the switch is one line, `engine.sim.positionReachable(w.floor, w.tileX)` in `walkerReachable`, plus a split-floor test case. Do this as soon as #652 is on main. Also noted: `towerCrowd.ts` and `sprites.test.ts` both sit near the 500-line ceiling; the walker-build split bought headroom on the first, the second will need one. `/gds-code-review`. |
