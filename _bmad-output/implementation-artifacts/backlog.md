...
| 2026-07-28 | segment-gripe-copy | #663 | gdd-modern-expansion | review-deferral | P3 | low | — | idea | **Deferred from `/gds-code-review` of the #647 segment-routing change (v1.103.0).** `unmetDemandGripeText` (`src/engine/sim/gripeCopy.ts`) still reads `floorReachable(u.floor)` rather than the segment-aware `positionReachable(u.floor, u.x)`. Shadowed in practice: a stranded-segment unit resolves to `noTransport` in `dominantGripe` before the unmet-demand branch, so the mismatched copy is never surfaced. Consistency-only; no player-visible defect and no golden impact. Fix: switch the read to `positionReachable` so the copy path stays segment-consistent if `dominantGripe` ordering ever changes. `/gds-code-review`. |
