...
Everything below this line is the one-time clearing pass adopted with the per-PR
cadence (see "How items flow"). Entries dated 2026-07-19 and older were each
folded into a curated row, marked absorbed or superseded with a pointer, or
retired with a reason; the leading blockquote on each section records its
disposition and the evidence is kept beneath it. The live inbox above this line
holds only the current in-flight window (2026-07-20). Counts are tallied in the
triage summary at the end of this section.
...
> **RETIRED (#545).** Making retail patronage crowd-emergent (rather than the surviving `rainMult`) is a demand-pool rearchitecture explicitly out of scope for a P3 texture pass, and `commercial-demand-pools` (#393) already shipped the statistical model; the attendance rain magnitude, the `weatherFor` layering smell, and the one-tick day-boundary weather lag are provisional/cosmetic and fold into the general playtest weight-tuning, no standing row.
