...
### Deferred from: `bmad-code-review` of the icon PR bulldoze/anchor pass (#733, 2026-08-02)

- **Bulletin icon swap assumes the mapped marker sits at index 0 (Edge Case Hunter + Blind Hunter, minor, latent).** `appendMessageWithIcons` / `messageWithIcons` (`src/ui/icons.ts`) match `LEADING_EMOJI_RE = /^(...)/`, so a mapped bulletin emoji only becomes an icon when it is the very first character. Verified safe today: every engine/UI bulletin string places its mapped marker at index 0 with exactly one mapped emoji, and `logLine`/`toast` pass `e.text` through with no prefix. The latent gap: if a future message ever prefixes the marker (a leading space, a `"Day 5: 🔥..."` label, a quote), the emoji renders as plain text and tofus again on a no-emoji-font system, the exact #721 failure, with no error. Fix shape if picked up: either add a coverage-guard assertion that every emitted bulletin-emoji literal is message-leading, or relax the mapper to skip leading whitespace/prefix while still swapping only the first marker. Not fixed now because no current message triggers it and index-0 is the engine's standing convention.
