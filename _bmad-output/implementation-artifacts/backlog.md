...
| 2026-07-31 | tdt-express-desync | #740 | SimTower parity | bug | P1 | high | — | open | **The retail 1994 game loses EVERY shaft written after an express slot.** Isolated on the Wine harness with minimal two-shaft towers, one screenshot each: express-then-standard loses the standard; standard-then-express keeps both; a standard shaft skipping 27 of its 30 floors keeps both. So it is neither stop-skipping nor span, it is the express record itself, and the express always renders correctly on its own. **Trigger isolated to the `type` byte:** flipping it from 0 (express) to 1 (standard) on an otherwise byte-identical file makes the following shaft render, so a `type = 0` record has a different SIZE than a type 1/2 one and our writer emits one size for all three kinds. Patching the capacity byte (42 → 21, the value a real save carried even for a service shaft) or the car count changes nothing, and six payload-size probes (+348, +9,720, +29,160, -348, -3,140, -9,720) all missed. The exact size cannot be brute-forced from the render: the game resumes reading 194-byte headers from wherever it thinks the payload ended and zero padding preserves that misalignment, so only the exact value works, leaving ~194 candidate offsets and nothing to bisect on. **Unblocked by a game-written save containing an express shaft** (how the 3,140-byte block was measured on my_tower); the harness `save` mode needs a genuine 16-bit `COMMDLG.DLL` at `tools/simtower/native/` first, or the save can be made by playing interactively. **Mitigated, not fixed:** `tdtEncoder.ts` now writes express shafts last (stable sort), measured to recover the owner's tower from a handful of rendered shafts to a dozen-plus. A tower with ONE express keeps every other shaft; TWO or more still lose everything after the first. This was the root cause behind the "some elevators are missing" report that opened this whole line of work. |
