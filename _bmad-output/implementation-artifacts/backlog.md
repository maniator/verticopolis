...
| 2026-07-21 | analytics-platform-decision | #542 | analytics | design-decision | P2 | — | — | open | **DECISION NEEDED (owner; audit §19 D-1): not approved work.** Stay on Vercel Web Analytics vs migrate to PostHog free tier in cookieless/memory-persistence mode. Audit evidence: aggregate-only limits drove most of #531/#533's complexity; traffic (~a dozen sessions/30 days) is trivially inside free tier; the empty depth tables make now the cheapest window. Auditor recommends migrate, cookieless-only, decision-record first, short dual-write; #533's percentile machinery becomes throwaway (accepted). The ruling also settles #538's payload shape, #528's exact-percentile path, and the #385/E1e platform dimension. No migration issue exists until this closes with a ruling. |
