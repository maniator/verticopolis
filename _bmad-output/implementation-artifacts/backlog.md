...
| 2026-07-29 | desktop-build-mode | #710 | mobile-distribution | review-deferral | P3 | low | — | open | **Gated.** Deferred from the `bmad-code-review` of the desktop build mode (PR #707 and follow-up #709). `resolvePlatformLabel` (`src/analyticsEnrichment.ts`) maps a port reporting `isNativeWrapper` to the analytics label `"ios"`, which PRD F8 and the architecture doc bind: both were written when the iOS Capacitor shell was the only wrapper, and their mode enumeration names neither desktop nor Electron. The desktop mode now lets an Electron wrapper bind a port through the same `__VC_PLATFORM__` seam with the same flag, so a desktop session satisfies the predicate the label keys on. Latent today: wrapped builds keep the telemetry host gate closed (`telemetryHostAllowed` is false for `native` and `desktop`), so a desktop session sends nothing and no event can be mislabeled today. Gated on desktop analytics being approved and opened, a separate decision. Fix shape: widen the label derivation to distinguish wrapper kinds (the port is the natural carrier) and update PRD F8 plus the architecture platform-dimension text together, since the docs are the binding authority. No engine, save, or gameplay impact. `/bmad-code-review`. |
