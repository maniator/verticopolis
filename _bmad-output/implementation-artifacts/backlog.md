...
| ---- | ----- | ---- | ---- | -------- | -------- | ----- | ------ | ----- |
| 2026-07-15 | elevator-dispatch-balancing | gameplay-feel | review-deferral | P2 | med | — | open | **Recovered defer, recorded 2026-07-15 (it had lived only in session memory since the crowd/vehicle read-right review round, PR #285 era; the owner approved it then as its own PR).** `bfsRoute` picks deterministically among equivalent shafts, so identical trips funnel onto ONE shaft of a bank: that landing queue piles up while sibling shafts sit idle, and the drawn crowd makes the imbalance obvious. Balancing route choice across equivalent shafts changes sim behavior AND the rng stream (golden-master churn), so it ships alone in a quiet golden-master window. SEQUENCING LAW (people-tracking priority party 2026-07-15, unanimous after rebuttal): the elevator queue-view reconciliation (the `pixelart-elevator-queue-seam` defer in the inbox below) lands FIRST, so the queue panel trusts exactly one population before dispatch changes move `carLoad`; otherwise a dispatch regression and a display artifact are indistinguishable. Full ruling: "People-tracking priority ruling" section at the end of this file. |
...
| 2026-07-08 | lunch-office-food-trips | SimTower parity | feature-request | P2 | med | — | done | **CLOSED 2026-07-15 (delivered by later work).** Delivered in two stages: the tower-wide meal cadence (v1.16.0) added the aggregate office->food->office flow at every meal window (breakfast/lunch/dinner/late-night, weighted origins, return lag), and the per-person round-trip work made it individual (the same worker leaves the office, eats at the venue via `mealVenueId` + dwell, and returns). Nothing of the original ask remains unbuilt; the surviving hotel-gate remainder is tracked on [[per-person-meal-round-trips]]. |
...
2. **`finish()` tripwire test:** the attendance tally has one decrement path
...
3. **Cinema occupancy display** ([[cinema-occupancy-display]]): re-scoped
...
4. **Unroutable-metro guard + bulletin hint** (metro v1.34.0 defers): gates the
...
5. **Elevator queue-view reconciliation** (the `pixelart-elevator-queue-seam`
   defer, E6-S7): pick ONE population for the panel (`crowd.carRiders` vs the
   ...
   this way).
6. **Census arrival edges** (PR #184 defers: over-capacity arrivals eat
...
7. **[[tdt-import-population-seed]]:** seed occupants on import (a real 1994
   tower reads ~96 against the 234 written in its own save).
8. **Metro as visit origin** (metro v1.34.0 defers): best payoff-to-effort on
...
9. **`elevator-dispatch-balancing`** (new row in the table above): own PR in a
   quiet golden-master window, after item 5 lands.
10. **Hotel meal gate:** the open half of [[per-person-meal-round-trips]].

Later: [[shopping-trips]] (parked; resurrect on measured starvation),
[[tdt-export-routing-tail]] (harness-gated), and the metro ridership economy
(needs its own spec and party). Confirmed parked on purpose: hall visit
weight, hotel-mingle sampling, [[venue-peak-hour-scaling]], and
[[w3-basement-depth]].
