...
| 2026-07-31 | tdt-express-desync | #740 | SimTower parity | bug | P1 | high | — | open | **FOUR CONTROLLED PROBES 2026-08-04 evening: the trigger is the express RECORD and it is NOT its length.** Each variant rewrote only the elevator table of one 23-shaft export, leaving every other byte and the file length untouched, then was loaded and re-saved by the game: **no express at all -> 21 of 21 kept** (so shaft COUNT was never the limit; the "8-shaft ceiling" reading is dead); **express last, span-sized (what we ship) -> 22 of 23**, losing only the second express; **express first, span-sized -> 1**; **express first, STOP-sized, i.e. exactly the 6,274-byte record the game itself writes -> 2**. Giving the express the game's own length still loses the 20 shafts behind it, so the entire "pick the right per-floor count" family is eliminated. What remains is the record's CONTENT (our per-floor entries are zero-filled where the game's own entries carry live queue state; harmless for standard/service), its stop bitmap, or the fixed block. Practical consequence settled: **express-last is the correct shipping answer, not a stopgap**, because it puts the unreadable record where nothing follows it. Also measured: a re-save of a re-save is a fixpoint (same 8 shafts, same table end, same 5,672-byte tail). Next: diff a game-written express record against ours field by field, and try our record with the game's per-floor entry CONTENT spliced in. **The 1994 game loses shafts after certain express records; the two sizings differ dramatically (corrected later the same day, 2026-08-04).** The 2026-08-04 "both sizings produce the same 8 shafts" note compared two files that BOTH carried a stop-sized express, so it tested nothing; retracted on the issue. The two variants also differ in express POSITION (the stop-sized files come from pre-mitigation and experimental encoders that do not sort express last), so the contrast proves the shipping path safe without isolating sizing as the sole variable. Re-run via game re-saves (`File > Save` needs no `COMMDLG.DLL`: a tower loaded from a path saves back over it): a SPAN-sized express sorted last (what we ship) keeps **22 of 23** shafts, losing exactly the second express, precisely what the modal promises; a STOP-sized express at slot 5 keeps only **slots 0-7** and additionally loses every stairway and the parking-connection count (the re-save's ~5.7 KB post-table region is all zeros). The mitigation (express last) is measured to be doing its job and stays. **Pinned:** the game WRITES the express stop-sized, `194 + 3140 + 324 * inSpanStops + 348` (6,274 for 8 stops over a 91-floor span; measured directly on one game-written save and independently via the EOF fit of the 384,658-byte re-save), and its own re-saves carry stray stop bits OUTSIDE a shaft's span, so readers must clamp to the span (ours does, v2.13.0). **Open:** no single read-side rule explains both outcomes above; two shafts AFTER the stop-sized express survive (rules out the game reading it span-sized) yet the cut lands three slots later (rules out a plain stop-sized read). Next probes: re-save the 8-shaft re-save (is it a fixpoint), a 10-standard-shaft tower with no express (does any cut appear), and a minimal stop-sized-express-first tower (where does the cut land). |
