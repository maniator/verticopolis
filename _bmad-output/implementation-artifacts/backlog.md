...
| 2026-07-31 | tdt-express-desync | #740 | SimTower parity | bug | P1 | high | — | open | **THE TRIGGER IS THE EXPRESS RECORD, AND IT IS NOT ITS LENGTH (four controlled probes, 2026-08-04 evening).** Each variant rewrote only the elevator table of one 23-shaft export, leaving every other byte and the file length untouched, then was loaded and re-saved by the game: **no express at all -> 21 of 21 kept**; **express last, span-sized (what we ship) -> 22 of 23**, losing only the second express; **express first, span-sized -> 1**; **express first, STOP-sized, i.e. exactly the 6,274-byte record the game itself writes -> 2**. The first row retires the earlier "8-shaft ceiling" reading (shaft COUNT was never the limit; that reading came from a file whose express sat at slot 5). The last two retire the whole "pick the right per-floor count" family, since the game's own record length still loses the 20 shafts behind it. What remains is the record's CONTENT (our per-floor entries are zero-filled where the game's own entries carry live queue state, which is harmless for standard/service shafts), its stop bitmap, or the 3,140-byte fixed block. **Settled:** express-last is the CORRECT shipping answer, not a stopgap, because it puts the unreadable record where nothing follows it; and the degraded read is a fixpoint (re-saving the 8-shaft re-save gives the same 8 shafts, same table end, same 5,672-byte tail). **Next probe:** diff a game-written express record against ours field by field, then splice the game's per-floor entry content into our record and see whether the shafts behind it survive. **History (superseded, kept so the reasoning is auditable):** a 2026-08-04 note claimed both sizings produced the same 8 shafts; both files in that comparison carried a stop-sized express, so it tested nothing, and it was retracted on the issue. A follow-up comparison was confounded by express POSITION as well as sizing; the probes above control for that. **Still pinned from those rounds:** the game WRITES the express stop-sized, `194 + 3140 + 324 * inSpanStops + 348` (6,274 for 8 stops over a 91-floor span, measured directly on one game-written save and independently via the EOF fit of the 384,658-byte re-save), and its re-saves carry stray stop bits OUTSIDE a shaft's span, so readers clamp to the span (ours does, v2.13.0). Getting game-written saves needs no `COMMDLG.DLL`: a tower loaded from a path saves back over it via `File > Save` with no dialog. |
