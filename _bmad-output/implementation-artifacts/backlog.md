...
| 2026-07-16 | weather-shapes-crowd | #430 | SimTower parity | feature-request | P3 | — | — | idea | **From the #424 review (attendance venues now earn from their live fill).** Weather affects commerce only through the `rainMult` income multiplier in `collectTrafficIncome`; the crowd sim is weather-blind, so `customersIn`, foot traffic, and attendance fill never drop in the rain. Model weather in the crowd layer so rain thins the people actually out and about (fewer spawns, or a shift toward indoor venues) and retail/attendance income follows from a thinner crowd rather than a bolt-on multiplier; then reconsider whether `rainMult` can shrink or retire so rain has one source of truth. Deterministic (no new RNG beyond the seeded crowd stream), Classic-faithful in magnitude. `/gds-code-review` (crowd + economy); player-facing, version bump when built. |
