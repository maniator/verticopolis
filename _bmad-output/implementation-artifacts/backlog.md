...
| 2026-07-07 | service-capacity-10 | OpenSkyscraper docs | feature-request | P1 | — | — | done | **Shipped 2026-07-07 (v1.9.5).** `TRANSPORT_CAPACITY.elevatorService` 16 → **10** per the decoded capacity byte in dfloer's spec (42/21/10; the 42 and 21 match our already-sourced numbers; our 16 was uncited). Value flows through dispatch/congestion/stats via `transportCarCapacity()` (no test hard-coded 16); added a `canon.test.ts` tripwire pinning all three car capacities. Release note: "canon fix: service elevators carry 10, as in 1994." |
