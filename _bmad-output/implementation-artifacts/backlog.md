...
| 2026-07-28 | move-in-gate-approximations | #648 | economy-satisfaction | review-deferral | P3 | low | — | idea | **Deferred from the `/gds-code-review` of the move-in sustainability gate (v1.100.0, spec-move-in-sustainability-gate-2026-07-23).** Three minor approximations parked for scope; the gate ships correct for the reported churn, and the congestion over-block the same review found was fixed inline (congestion is neutralized in the gate context). (1) The probe household is fixed at the mean of 3, so a below-mean Modern buyer (2 people) relying on a daycare halo to offset noise can still churn (under-block); fix: probe the smallest household for the halo term, or fold into horizon calibration. (2) `attemptMoveIns` reuses one `satCtx` across a pass, so later same-pass candidates are judged on pre-fill occupancy/coverage (under-block on unmet demand); deterministic and self-heals the next tick; fix: rebuild the gate demand map mid-pass or per candidate (perf cost + golden re-pin). (3) `wontLeaseText` derives its verdict from a fresh demand map and its cause from the hour-memoized `sim.demandMap()`, so a narrow window can drop the specific "too few shops" cause to the generic string (inspector-only, fallback handles it); fix: thread one map through both. A floor-1 served-check divergence is noted but currently unreachable. `/gds-code-review`. |
