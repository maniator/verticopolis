...
| 2026-07-31 | tdt-express-desync | #740 | SimTower parity | bug | P1 | high | — | open | **The retail 1994 game keeps only the first 8 shafts of a 23-shaft export, and re-saves only those.** Originally read as "loses every shaft after an express slot" from minimal two-shaft towers. **Measured 2026-08-04 (Wine harness, a save the game itself wrote):** the game WRITES an express payload sized by its SERVICED floors, `194 + 3140 + 324 * stops + 348` (6,274 bytes for an express spanning 91 floors and stopping at 8), while standard and service match the spanned rule exactly. Getting that save needed no `COMMDLG.DLL` after all: a tower loaded from a path saves back to it via `File > Save` with no dialog. **Importer fixed (v2.13.0):** it could not read a game-written file with a skip-stopping express at all (lost the elevator table, the stairways and the parking count together); it now reads one clean, and the trailer keeps our own span-sized exports apart from the game's. **Exporter deliberately unchanged:** exporting the same tower with a stop-sized express and with a span-sized one produces the SAME 8 shafts in the game, so our express size is not what truncates the table, and the "write express last" mitigation stays until the real cause is known. Relocating the tower's two duplicate columns does not move the cut either. Next probes: whether the ceiling is 8 shafts or something about the 9th record, and whether a game-BUILT 9th shaft survives its own re-save. |
