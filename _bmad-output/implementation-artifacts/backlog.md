...
4. `Priority` is the do-first order (impact × effort × risk, and whether the
   ...
   spec. Rows are kept sorted P1 → P3.

| Date | Story | Epic | Type | Priority | Severity | Owner | Status | Notes |
| ---- | ----- | ---- | ---- | -------- | -------- | ----- | ------ | ----- |
| 2026-07-06 | condo-modes | Modern condos | review-deferral | P1 | medium | — | open | A `null`/malformed unit entry in a forged save crashes `deserialize` (pre-existing). The unit loop does `(data.units ?? []).filter((u) => isFacilityKind(u.kind))` — reading `u.kind` on a `null` entry throws and aborts the whole load, turning a recoverable save into a hard failure. Predates the condo work and is orthogonal to it. Well-scoped fix: guard the unit and transport filters with `u != null &&` (same for `data.transports`). (Blind Hunter, condo-modes final pass; forged/corrupt-save only.) |
