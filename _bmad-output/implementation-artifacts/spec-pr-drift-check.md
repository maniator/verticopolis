...
**Execution:**
- [x] `.github/workflows/pr-drift-check.yml` -- add the path-gated `pull_request` workflow: a `capture` job that calls the reusable `screenshot-capture.yml` (which renders each shard TWICE and hard-fails on any per-shard run-a != run-b, reusing `screenshot-shards.ts`), a `verify-drift` job that assembles the verified shard sets over the committed gallery and outputs `has_drift`, and a `commit-on-approval` job gated on the `screenshot-approval` environment that commits the regenerated gallery to the PR branch only when drift exists and a reviewer approves.
- [x] `commit-on-approval` -- assemble + commit mirrors the marker workflow's commit shape (rm-only-PNGs rebuild, `GITHUB_TOKEN`, fast-forward-guarded rebase-retry push); the `verify-drift` job posts/updates one sticky PR comment (best-effort) pointing the author at the pending approval when drift is detected.
...
- **2026-07-13, Codex PR review on the reusable refactor (two P2 findings, both fixed):** (1) The a/b-in-shard collapse rendered ONE shared build twice, so build-time / startup nondeterminism (a value baked into the bundle at build) would show identically in both legs, pass the hard check, then churn on the next run: the exact class this whole effort removes. Fixed by giving each leg its OWN `npm run build` + preview server (on its own port, `--strictPort`), which restores the coverage the two-job matrix had. The party's "same strength, half the build overhead" premise was corrected: a/b-in-shard saves the second checkout + `npm ci`, not the build. (2) The path gate omitted the version inputs that reach the splash; added `package.json` and `vite.config.ts` (`__APP_VERSION__` -> `00-splash`). `package-lock.json` deliberately left out (a Playwright/font bump is the `[update-screenshots]` marker's job; gating it would regen on every dependency PR). KEEP across any re-derivation: independent builds per a/b leg; version inputs in the gate.
...
Reuse vs DRY: the sharded, self-verifying capture is a reusable `workflow_call` (`screenshot-capture.yml`) that BOTH this workflow and `update-screenshots.yml` call, so there is one place that defines how pixels are generated and how nondeterminism is caught. The determinism guard renders each shard TWICE in the SAME container, each leg from its OWN `npm run build` + preview server (on its own port), and diffs the two renders (a/b-in-shard), rather than a `run:[a,b]` matrix of separate jobs. Independent builds per leg matter: a shared build rendered twice would show a build-time-baked value (a timestamp or RNG folded into the bundle) identically in both legs, pass, then churn next run (a Codex finding on this PR). Independent builds restore that coverage, so the guard is as strong as the two-job matrix; what a/b-in-shard actually saves vs that matrix is the second checkout + `npm ci` (4 shard jobs, not 8), NOT the build. Wall-clock grows to ~2x the slowest shard's build+render (acceptable for a background check). If that ever bites, the follow-up is dep-cache, not fewer builds.
