...
- **Housekeeping fields six time-simulated maids per unit (canon, as of 1.54)**: each unit staffs 6 maids, at most one of a unit's maids works a floor at a time, maids travel only by **service elevator or stairs** (escalators dropped from the staff network, canon), and each room takes real minutes of in-room cleaning before it turns over, so per-unit throughput is emergent from placement and staff transport instead of a fixed rooms-per-crew quota. **Classic** works the canon **noon-5 shift with a 16:30 no-new-room cutoff**; **Modern** works a longer 08:00-19:00 staffed day (a rule-set divergence, like the exterminator). What we still deliberately do not reproduce: the 1994 pathfinding quirk (seven-plus floors degraded) that pushed players to build one service elevator per six floors; our staff routing has no such degradation.
