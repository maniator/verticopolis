...
// rough capture cost so the shards finish together (migration + milestones are
// ~50s/shot; showcase carries the most shots). This is the ONLY place the split
// is defined; correctness is enforced by `verify` against SCENES, so a rebalance
// here is safe as long as `verify` still passes.
export const SHARDS: Record<string, string[]> = {
  showcase: ["showcase", "mobile"],
  slow: ["migration", "milestones"],
