 ...
 *
 * The property being protected lives in the BUILT artifact, not in the source.
 * `hostCommands.ts` exists to serve a native shell's application menu, so a
 * browser bundle should not carry it at all: every player who loads the web
 * game would otherwise download a module that can never do anything. Source
 * tests cannot see this. `IS_WRAPPED_BUILD` is `false` under vitest either way,
 * which proves nothing about whether Rollup actually eliminated the branches
 * and dropped the module.
 *
...
const MARKERS = ["Start or load a tower first", "Close the open window first", "Ignoring unknown host command"];
...
for (const marker of MARKERS) {
  const hit = files.some((f) => readFileSync(join(ASSETS, f), "utf8").includes(marker));
  if (hit) found.push(marker);
}

if (shouldBePresent && found.length !== MARKERS.length) {
...
if (!shouldBePresent && found.length > 0) {
  fail(
...
console.log(
  `verify-wrapper-seam: ${arg} build is correct (command seam ${shouldBePresent ? "present" : "absent"}, ` +
    `${files.length} bundle${files.length === 1 ? "" : "s"} scanned)`,
);
