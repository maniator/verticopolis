/**
 * v1 → v2 save migration: the segment-parity reflow that re-lays each floor's
 * rooms at their canon (post-E1b) widths, plus the validity guards that let
 * `upgradeV1toV2` fall back to the un-reflowed save. Extracted from
 * `saveMigration.ts`; pure functions on `SerializedGame`.
 */
