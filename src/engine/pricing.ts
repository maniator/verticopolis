 ...
 * 2026-07-22 the GameFAQs FAQ lineage (BStuart, furdude2, Aristotle47)
 * independently corroborated every table below, rung for rung, including the
 * nightly hotel cadence (spec-classic-economy-canon-cadence-2026-07-22 §2).
 * The archive.org manual remains unread; if it ever contradicts these, re-open
 * there. Classic uses the FULL canon values by the owner's call of 2026-07-08.
 */
const CLASSIC_RENT_LADDERS: Readonly<Partial<Record<string, readonly [number, number, number, number]>>> = {
  // Office, quarterly. HARD confidence: matches our band anchors; full ladder
  // corroborated by the GameFAQs lineage (2/5/10/15k quarterly).
  office: [2_000, 5_000, 10_000, 15_000],
  // Condo, one-time sale (locked after it sells). MED confidence: the $150k
  // one-time figure is corroborated (GameFAQs lineage), but a 40k-vs-50k
  ...
  // Hotel single, nightly. MED confidence: was single-source (~10x our old
  // band, accepted per GDD §2), now corroborated by the GameFAQs lineage
  // (500-1500-2k-3k nightly).
  hotelSingle: [500, 1_500, 2_000, 3_000],
  // Hotel double, nightly. MED confidence, corroborated (800-2k-3k-4500).
  hotelDouble: [800, 2_000, 3_000, 4_500],
  // Hotel suite, nightly. MED confidence, corroborated (1500-4k-6k-9k).
