...
describe("app_action telemetry", () => {
  ...
  it("reports app_action with just the action when no detail is given", () => {
    ...
    expect(track).toHaveBeenCalledWith("app_action", { action: "quick_save" });
  });

  it("includes the detail dimension when given", () => {
    trackAppAction("mute", "on");
    expect(track).toHaveBeenCalledWith("app_action", { action: "mute", detail: "on" });
  });
  ...
  it("latches trackAppActionOnce to a single emission per session", () => {
    ...
    trackAppActionOnce("volume");
    trackAppActionOnce("volume");
    const volumeCalls = vi.mocked(track).mock.calls.filter(([name, props]) => name === "app_action" && (props as { action?: string }).action === "volume");
  ...
  it("latches each action independently", () => {
    ...
    expect(track).toHaveBeenCalledTimes(2); // two distinct latched actions
    expect(track).toHaveBeenCalledWith("app_action", { action: "mute", detail: "on" });
  });
