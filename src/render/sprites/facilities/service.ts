 ...
 *
 * Each look is ported from its Figma reference build script
 ...
 * `RW`/`RH` equal the on-screen footprint and `refMap` is the identity. Keep it
 * that way: off the shipped size every rect resamples, and a resampled repeat
 * (the monitor wall, the sign letters, the bottle rows) loses its even pitch.
 */
...
export function drawSecurity(d: DrawCtx, x: number, y: number, w: number, h: number): void {
  ...
  // Two-by-five wall of green-dot camera monitors. Ten copies of one object, so
  // they take the uniform fill and a pitch that divides evenly: 7 wide on a 9
  // pitch leaves the same 2px gutter between every pair, across and down.
  const monPitch = 9;
  const monW = monPitch - 2;
  const monH = 9;
  for (let r = 0; r < 2; r++) {
    for (let cN = 0; cN < 5; cN++) {
      const mx = 6 + cN * monPitch;
      const my = 6 + r * (monH + 2);
      box(Fu, mx, my, monW, monH, "#0E1420");
      Fu(mx + 1, my + 1, monW - 2, 5, "#1A3A2A");
      Fu(mx + 1 + ((r + cN) % 3), my + 2, 1, 3, "#6bd47a", 0.8);
      Fu(mx + 1, my + 1, monW - 2, 1, "#2A4A3A");
    }
  }
  // Console desk with a seated guard behind it.
  const deskX = 53, deskW = 24;
  box(F, deskX, fy - 8, deskW, 8, "#2A3550");
  F(deskX, fy - 8, deskW, 1, "#3E4A66");
  personSeated(ctx, sx(deskX + 7), sy(fy - 8), (Math.round(x) * 3 + 5) | 0);
  // Brass badge shield, key rack, and the red alarm light.
  F(deskX + 2, fy - 14, 6, 6, "#D8B05A");
  F(deskX + 4, fy - 13, 2, 4, "#8A6A2A");
  const rackX = 70;
  F(rackX, 8, 8, 5, "#5A4A36");
  for (let k = 0; k < 4; k++) Fu(rackX + 1 + k * 2, 13, 1, 2, "#D8B05A"); // key rack pins, one object repeated
  F(W - 8, 7, 4, 4, "#E85D5D");
  glow(F, W - 6, 9, "#FF6B6B");
...
export function drawMedical(d: DrawCtx, x: number, y: number, w: number, h: number): void {
  ...
  for (let lx = 22; lx < W; lx += 40) {
    F(lx, 2, 9, 2, lamp);
    glow(F, lx + 4, 5, lamp);
  }
  // Red cross sign. Both bars are centered on the panel, so the cross reads
  // square rather than leaning a pixel.
  F(5, 8, 13, 13, "#F4F0EC");
  F(9, 10, 5, 9, "#D6342F");
  F(6, 12, 11, 5, "#D6342F");
  // First curtained exam bed with a resting patient head, blanket, and rail.
  const bedW = 24;
  box(F, 25, fy - 9, bedW, 9, "#DCE2E8");
  F(25, fy - 9, bedW, 1, "#F0F4F8");
  F(27, fy - 12, 6, 3, SKIN[0]);
  F(27, fy - 13, 6, 1, "#3A2E28");
  F(32, fy - 10, 15, 3, "#BFD0DE");
  F(24, 10, 1, fy - 16, "#B8BCC0");
  F(24, 10, 25, 1, "#B8BCC0");
  F(49, 12, 4, fy - 18, "#9FC0C8", 0.6);
  // Heart monitor with a green trace.
  box(F, 58, fy - 16, 13, 10, "#20242C");
  F(60, fy - 14, 9, 6, "#0E241A");
  F(60, fy - 11, 2, 1, "#6bd47a");
  F(62, fy - 12, 1, 3, "#6bd47a");
  F(63, fy - 10, 2, 1, "#6bd47a");
  F(65, fy - 13, 1, 5, "#6bd47a");
  F(66, fy - 11, 3, 1, "#6bd47a");
  glow(F, 65, fy - 11, "#3ADF6A");
  // IV stand.
  F(55, fy - 16, 1, 16, "#B8BCC0");
  F(53, fy - 18, 5, 3, "#CFE0FF", 0.8);
  ...
  // Stocked supply cabinet, sized so the five bottle pitches clear both walls
  // by the same margin.
  ...
  F(120, fy - 11, 5, 4, "#4A5464");
  F(120, fy - 2, 3, 3, "#1A1D24");
  F(125, fy - 3, 2, 2, "#1A1D24");
  // Second exam bed.
  box(F, 131, fy - 9, bedW, 9, "#DCE2E8");
  F(133, fy - 12, 6, 3, SKIN[1]);
  F(133, fy - 13, 6, 1, "#3A2E28");
  F(138, fy - 10, 15, 3, "#BFD0DE");
}
...
export function drawHousekeeping(d: DrawCtx, x: number, y: number, w: number, h: number): void {
  ...
  // Linen shelf. The four stacks are placed off the carcass width, so every
  // shelf keeps the same end margin and the same gap.
  const shelfX = 4, shelfW = 27;
  box(F, shelfX, 6, shelfW, fy - 8, "#9A8468");
  ...
  box(F, cartX, fy - 10, cartW, 8, "#3E8E8E");
  for (let k = 0; k < 2; k++) F(cartX + 2 + k * 6, fy - 14, 5, 4, k ? "#CFE0FF" : "#F4F0EC");
  for (let k = 0; k < 2; k++) F(cartX + 12 + k * 3, fy - 13, 2, 3, k ? "#5db4e8" : "#E85D5D");
  F(cartX, fy - 2, 2, 2, "#1A1D24");
  F(cartX + cartW - 2, fy - 2, 2, 2, "#1A1D24");
  // Mop and bucket.
  F(62, fy - 14, 1, 12, "#8A5A30");
  F(60, fy - 16, 5, 3, "#E8E0B0");
  box(F, 60, fy - 6, 6, 6, "#F4C020");
  F(60, fy - 6, 6, 2, "#CFE0FF", 0.6);
  // Towel rack: four bars on a 3px pitch cut the carcass into five equal reveals.
  const rackX = W - 20, rackW = 14;
  box(F, rackX, fy - 9, rackW, 9, "#B8A47E");
  for (let k = 0; k < 4; k++) F(rackX + 2 + k * 3, fy - 9, 1, 9, "#9A8460");
  F(rackX + 2, fy - 11, 10, 3, "#F4F0EC");
  // Standing housekeeper by the cart (the teal cart carries the uniform read).
  personStanding(ctx, sx(35), sy(fy), (Math.round(x) * 3 + 3) | 0);
  ceilingFixture(ctx, x, y, w, d.lit);
}
...
export function drawRecycling(d: DrawCtx, u: Unit, x: number, y: number, w: number, h: number): void {
  ...
  // studs and pipe brackets are repeated objects and take the uniform one. The
  // rib pitch divides the hall exactly, so the bay spacing cannot drift.
  const ribPitch = W / 10;
  F(0, 0, W, H, "#6C6C64");
  F(0, 0, W, Math.round(H * 0.5), "#75756B");
  for (let px = 0; px < W; px += ribPitch) Fu(px, 2, 1, deck - 2, "#5A5A52");
  ...
  for (const lx of [42, 136]) {
    F(lx, 10, 1, 3, "#3A3E44");
    F(lx - 3, 13, 7, 3, "#2A2E34");
    F(lx - 2, 15, 5, 1, "#F8E2B4");
    glow(F, lx, 18, "#F8E2B4");
  }
  // Overhead sorting sign. The glyph run is centered off the board width, so
  // the eleven letters keep matching margins at both ends.
  const signX = 84, signW = 63, glyphs = 11, glyphW = 3, glyphPitch = 5;
  const glyphInset = Math.round((signW - ((glyphs - 1) * glyphPitch + glyphW)) / 2);
  box(F, signX, 5, signW, 10, "#2A6E3A");
  for (let i = 0; i < glyphs; i++) Fu(signX + glyphInset + i * glyphPitch, 8, glyphW, 4, "#DCE8C0"); // sign letters
  // Conveyor belt carrying baled recyclables (no reserved red bale). The bale
  // run is bounded by the belt, so a narrower belt drops bales, never crowds them.
  const by = deck - 24;
  const beltX = 24, beltW = 109;
  F(beltX, by, beltW, 6, "#3A3E44");
  F(beltX, by, beltW, 1, "#4E545C");
  F(beltX, by + 5, beltW, 1, "#242830");
  for (let bx = beltX + 2; bx + 3 <= beltX + beltW - 2; bx += 6) Fu(bx, by + 1, 3, 4, "#2E323A"); // baled recyclables
  F(beltX - 2, by - 1, 4, 8, "#4A4E54");
  F(beltX + beltW - 2, by - 1, 4, 8, "#4A4E54");
  const items = ["#4E7A4A", "#3E6A9E", "#C8A24A", "#C87A32", "#DCDCE0", "#8C5A3A"];
  for (let ix = beltX + 4, k = 0; ix + 5 <= beltX + beltW; ix += 12, k++) {
  ...
  for (let k = 0; k < 2; k++) F(cx + 4 + k * 8, by - 10, 6, 4, "#3A3E44");
  box(F, cx + 23, by - 15, 15, 20, "#565C64");
  for (let k = 0; k < 2; k++) F(cx + 25 + k * 7, by - 12, 4, 4, k ? "#E85D5D" : "#3ADF6A");
  F(cx + 25, by - 6, 11, 4, "#1A1D24");
  F(cx + 26, by - 5, 9, 2, "#5FB0DC");
  // Three sorting bins.
  ...
  bins.forEach((c, i) => {
    ...
    box(Fu, bx, deck - 16, binW, 16, c[0]);
    Fu(bx, deck - 16, binW, 2, c[1]);
    Fu(bx + 5, deck - 19, binW - 10, 3, c[1]);
  ...
  for (let r = 0; r < 2; r++) {
    for (let c = 0; c < 2; c++) {
      ...
      box(F, bx2, by2, 16, 7, "#B08A5A");
      F(bx2, by2 + 3, 16, 1, "#8A6A44");
  ...
  // Painted deck curb. The 4px stripe unit divides the hall exactly, so the
  // hazard tape ends on a whole stripe instead of a clipped sliver.
  F(0, deck, W, 7, "#4A4A44");
  F(0, deck, W, 1, "#5E5E56");
  for (let i = 0; i < W; i += 4) {
    Fu(i, deck + 1, 2, 2, "#D8B23A");
    Fu(i + 2, deck + 1, 2, 2, "#2A2418");
  }
  F(18, deck + 3, W - 36, 1, "#000000", 0.2);
