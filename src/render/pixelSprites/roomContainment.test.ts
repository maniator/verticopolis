...
/**
 * Every finalized figure, as the baseline it stands on and the columns its torso
 * covers. A person build stamps one contact shadow per figure, a 1px rect one
 * pixel wider than the torso on each side, and nothing else uses that literal,
 * so the shadow locates the figure and states its width without the drawing code
 * having to report anything.
 *
 * The shadow's own span is deliberately not what is measured: neighbors at a
 * shoulder-to-shoulder pitch are drawn with their shadows merging, which is how
 * a crowd is supposed to read.
 */
function figures(log: string[]): { foot: number; x0: number; x1: number }[] {
  const out: { foot: number; x0: number; x1: number }[] = [];
  log.forEach((l, i) => {
    if (l !== "fillStyle=rgba(0,0,0,0.24)") return;
    const rect = log.slice(i + 1).find((r) => r.startsWith("fillRect:"));
    if (rect === undefined) return;
    const [x, foot, w] = JSON.parse(rect.slice("fillRect:".length)) as number[];
    out.push({ foot, x0: x + 1, x1: x + w - 2 }); // torso columns, shadow margin off each end
  });
  return out;
}
...
const columnsFor = (over: Partial<Unit>): number[] => (over.kind === "office" ? officeColumns : [0, 1, 2, 3]);
...
describe("a full room's figures stand beside each other, never through each other", () => {
  // The other half of packing the restored slots back in. Containment proves a
  // row does not reach past the wall; this proves it does not solve the count by
  // stacking people on each other instead.
  //
  // The failure it guards is one step removed from any room's own code. A figure
  // is fixed-size pixel art that did not shrink when the tile did, so scaling a
  // row's pitch with the tile spends the gap the art was drawn with, and a row
  // authored shoulder to shoulder has no gap to spend: the club's 6px dancers at
  // a 6px pitch came out at a 5.45px step and overlapped by a pixel in half the
  // gaps. Nothing else could see it. Capacity counted the right number of
  // dancers and containment found them all inside the room, because two people
  // drawn a pixel into each other are still the right count in the right box.
  //
