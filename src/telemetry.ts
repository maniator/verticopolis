...
/**
 * Shared Core Web Vitals inject for every deployed page: the game boot
 * (`bootstrap.ts`), the sprite gallery (`gallery.ts`), and the standalone
 * `/help` page (`helpPage.ts`). It reports page performance through the one
 * analytics adapter (Vercel Speed Insights, kept at the D-1 cutover; the Web
 * Analytics page-view inject retired with the rest of that path), but only
 * where the Vercel endpoints (`/_vercel/speed-insights/*`) actually exist: the
 * production domain and Vercel preview deployments.
 *
