...
export function statsTemplate(sim: Simulation): TemplateResult {
  ...
  // maids-times-anchor capacity is only the fallback before the first checkout
  // (fresh game or load), when no observed figure exists yet; it is an
  // optimistic best case, so it never subtracts the infested rooms out of the
  // workload (that discount is how the old readout said "enough" while a wing
  // rotted).
  const hkReport = sim.economy.housekeepingReport();
  const hkShort =
    hk.rooms > 0 &&
    (hk.infested > 0 || hk.outOfReach > 0 || (hkReport ? hkReport.leftover > 0 : hk.dailyCapacity < hk.rooms));
  ...
  return html`<div class="stats-grid">
      ...
      <div class="col kv">
        ...
        ${
          hk.rooms > 0
            ? html`<span class="k">Housekeeping</span><span class="v" style="color:${hkShort ? "var(--bad)" : "var(--good)"}">${fmt(hk.crews)} crew${hk.crews === 1 ? "" : "s"} (${fmt(hk.maids)} maids) for ${fmt(hk.rooms)} room(s)${hkReport && hkReport.leftover > 0 ? `, ${fmt(hkReport.leftover)} unserved yesterday` : ""}${hk.outOfReach > 0 ? `, ${fmt(hk.outOfReach)} out of reach` : ""}</span>`
            : nothing
        }
        ${
          hk.infested > 0
